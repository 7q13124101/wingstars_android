...
class FrequentlyAskedQuestionsViewModel : ViewModel() {
    ...
    var questionRegisterData = MutableLiveData<MutableList<FrequentlyQuestionsResponse.Data.GroupDto>>()
    var questionDownloadData = MutableLiveData<MutableList<FrequentlyQuestionsResponse.Data.GroupDto>>()
    ...
    fun getFrequentlyAskedQuestionsData() {
        ...
        API.shared?.api?.let {
            val observer = it.nsQuestions("${BaseApplication.HOST_HAWKS_CDN}/api/v1/app/questions")
...
//                            Log.d("question",i.toString())
                            ...
                            when (i.partName) {
                                BaseApplication.shared()!!.getString(R.string.user_points_task) ->
                                    questionTaskData.postValue(groups)
                                BaseApplication.shared()!!.getString(R.string.user_register_login) ->
                                    questionRegisterData.postValue(groups)
                                BaseApplication.shared()!!.getString(R.string.user_download_and_install) ->
