...
import com.wingstars.user.utils.GetJsonDataUtils
...
class PolicyTermModel : ViewModel() {
    ...
    fun getPrivacyPolicyJson(context: Context) {
        val jsonDataStr = GetJsonDataUtils().getJson(context, "privacy_policy.json")
//        Log.d("PolicyTerm", "jsonDataStr = $jsonDataStr")
    ...
    fun getUserTermsJson(context: Context) {
        val jsonDataStr = GetJsonDataUtils().getJson(context, "user_terms.json")
